...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 7) {
                        found === true 
                        aruga.reply(from, '*[ANTI STICKER SPAM]*\nYou have SPAM STICKER in the group, you will be kicked automatically by the bot', message.id).then(() => {
                            ...
                            if (found !== false) {
                                ...
                                aruga.sendText(from, result)
                                    ...
                                    aruga.reply(from, `${monospace(`There isn't a number in the database`)}`, id)
                            }
                        })
                        return true;
                    }else{
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, '*[GROUP LINK DETECTOR]*\nYou sent a group chat link, sorry you got kicked from the group:(', id).then(() => {
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'Successfully convert Sticker to Image!', id)
                .then(() => {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                if (isMedia || isQuotedImage) {
                    ...
                    console.log(err)
	   	            await aruga.reply(from, 'Sorry, todays usage limit has reached the maximum', id)
                    }
                }
	...
	case 'brainly':
        ...
        case 'stickergiphy':
            ...
            break
      case 'qrread':
        if (args.length !== 1) return aruga.reply(from, `To use the qrread feature, Type :\n${prefix}qrread url\n\nExample: ${prefix}qrcode https://i.ibb.co/phSpp2h/00bed2bb-8b90-4d49-ace1-fe0ac9f73dff.jpg\n\n*Note : First upload your qrcode to https://id.imgbb.com/, then copy your qrcode image url*`, id)
      ...
      break
    case 'qrcode':
        if (args.length !== 2) return aruga.reply(from, `To use the qrcode feature, Type :\n${prefix}qrcode [said/url] [size]\n\nExample: ${prefix}qrcode https://google.com 300\n\n*Size minimal 100 & maximum 500*`, id)
        aruga.reply(from, `wait...`, id);
      ...
      break			
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                await aruga.reply(from, `No picture! Please send a picture with a caption ${prefix}meme <text_up> | <text_down> \ nexample: $ {prefix} top text meme | bottom text`, id)
            }
            break
        case 'quotemaker':
	...
	//Group All User
	case 'grouplink':
            if (!isBotGroupAdmins) return aruga.reply(from, 'This command can only be used when the bot becomes admin', id)
            if (isGroupMsg) {
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'This command can only be used when the bot becomes admin', id)
                    if (isBotGroupAdmins) {
                        aruga
                            ...
                            .then((res) => {
                                aruga.reply(from, `Successfully use Revoke Group Link*${prefix}grouplink* to get the latest group invite link`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        case 'ytmp3':
            ...
            .then(async(res) => {
				...
				.catch(() => {
					aruga.reply(from, `This URL ${args[0]} It has been downloaded before. URL will be reset after 1 hour / 60 minutes`, id)
				})
			})
            break
        case 'ytmp4':
       ...
       case 'moddroid':
        ...
        case 'fml':
            ...
            .catch(() => {
                aruga.reply(from, 'Hayolohhh, Something went wrong!!', id)
            })
            break
      	case 'motivasi':
        ...
        case 'memes':
            ...
            .catch(() => {
                aruga.reply(from, 'Something Error!', id)
            })
            break
	...
	case 'dewabatch':
		...
		break
        case 'images':
            if (args.length == 0) return aruga.reply(from, `To search for images from pinterest\nType: ${prefix}images [search]\nExmample: ${prefix}images naruto`, id)
            ...
            break
        case 'sreddit':
            if (args.length == 0) return aruga.reply(from, `To search for images from the reddit sub,\n type: ${prefix}sreddit [search]\nExample: ${prefix}sreddit naruto`, id)
        ...
        case 'lyrics':
        case 'lirik':
            if (args.length == 0) return aruga.reply(from, `To search for lyrics\n Type: ${prefix}lyrics [tum sath ho]`, id)
            rugaapi.lirik(body.slice(7))
            .then(async (res) => {
                await aruga.reply(from, `Song Lyrics: ${body.slice(7)}\n\n${res}`, id)
            })
            break
        case 'chord':
            ...
            break
        case 'ss': //if an error, please open the file in the settings / api.json folder and change the apiSS 'API-KEY' that you got from the website https://apiflash.com/
            if (args.length == 0) return aruga.reply(from, `Make the bot screenshot a website \n\nUsage: ${prefix}ss [url]\n\nExample: ${prefix}ss http://google.com`, id)
        ...
        case 'play'://silahkan kalian custom sendiri jika ada yang ingin diubah
            ...
            break
		case 'movie':
			if (args.length == 0) return aruga.reply(from, `To search for a movie from the sdmovie website.fun\nketik: ${prefix}movie Movie name`, id)
        ...
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                    ...
                    teks += `➸ *Similar* : ${(similarity * 100).toFixed(1)}%\n`
